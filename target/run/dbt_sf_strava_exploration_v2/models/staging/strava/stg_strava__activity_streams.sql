        ...
        create or replace transient table STRAVA_PROD.staging.stg_strava__activity_streams
         as
        (-- steps
...
with activity_streams_raw as (
    select *
    from STRAVA_PROD.raw.strava__activity_streams
...
, activity_streams_with_keys_and_metadata AS (
    SELECT 
        *,
        ...
        md5(cast(coalesce(cast(activity_key as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(elapsed_time_s as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as activity_stream_key,
