-- time in power zones (cycling only)

-- methodology
-- - filter cycling activities, with power data
-- - get activity streams for activities, filtering out static records
-- - get power zones for each activity
-- - calculate (moving) time in power zones from activity streams for each activity

-- caveats
-- - moving time across all power zones does not cover all moving time due to gaps in streams

-- tests
-- uniqueness: time in power zone key (activity key, power zone number)
-- nullability: N/A
-- allowed values: N/A
